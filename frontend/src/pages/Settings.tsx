/* 设置页面 - Apple Clean Style */

import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { motion } from 'framer-motion';
import { ArrowLeft, Key, Eye, EyeOff, CheckCircle, AlertCircle, Plus, Trash2, Globe, Wrench, XCircle } from 'lucide-react';
...
const Settings: React.FC = () => {
  ...
  const updateProvider = (id: string, field: keyof LLMProviderConfig, value: string) => {
    setConfig((prev) => ({
      ...prev,
      [id]: { ...{ api_key: '', base_url: '', model: '' }, ...prev[id], [field]: value },
    }));
  };

  const removeProvider = (id: string) => {
    setConfig((prev) => { const next = { ...prev }; delete next[id]; return next; });
  };
  ...
  const handleSave = () => {
    ...
    Object.entries(config).forEach(([id, cfg]) => {
      if (cfg.api_key && cfg.api_key.trim()) {
        cleaned[id] = { ...cfg, api_key: cfg.api_key.trim() };
      }
    });
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-page)' }}>
      <header style={{
        ...
        <div style={{ maxWidth: 'var(--content-max)', margin: '0 auto', padding: '0 var(--side-padding)', height: 'var(--nav-height)', display: 'flex', alignItems: 'center', gap: 'var(--space-4)' }}>
          <motion.button onClick={() => navigate('/')} whileTap={{ scale: 0.97 }}
            style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: 'var(--accent)', fontSize: 'var(--text-footnote)', display: 'flex', alignItems: 'center', gap: '4px' }}>
            <ArrowLeft style={{ width: 16, height: 16 }} /> Back
          </motion.button>
          <h1 style={{ fontSize: 'var(--text-heading)', fontWeight: 'var(--fw-semibold)', color: 'var(--text-primary)', letterSpacing: 'var(--ls-feature)', margin: 0 }}>LLM Configuration</h1>
        </div>
      </header>
      ...
      <main style={{ maxWidth: 'var(--content-max)', margin: '0 auto', padding: 'var(--space-8) var(--side-padding) var(--space-24)', display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
        ...
        {!configuredCount && (
          <Card className="border-accent-green/15 bg-accent-green-dim">
            <CardContent className="p-4">
              <div className="flex items-center gap-2 text-accent-green text-13">
                <CheckCircle className="w-4 h-4" />
                <span className="font-medium">当前使用演示模式（Mock）</span>
              </div>
            </CardContent>
          </Card>
        )}
            ...
            <Card key={id}>
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    <CardTitle className="text-15 flex items-center gap-2">
                      <Key className="w-4 h-4 text-secondary" />
                      {preset?.name || id}
                    </CardTitle>
                    <CardDescription>{preset?.desc || '自定义 OpenAI 兼容提供商'}</CardDescription>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    {cfg.api_key && cfg.model && (() => {
                      ...
                      const hasImageGen = caps.includes('image_generation');
                      if (!modelSupportsTools(id, cfg.model)) {
                        return (
                          <span className="inline-flex items-center gap-1 text-11 bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full font-medium" title="此模型不支持 Function Calling，工具不可用">
                            <XCircle className="w-3 h-3" /> 无工具支持
                          </span>
                        );
                      }
                      if (hasImageGen) {
                        return (
                          <span className="inline-flex items-center gap-1 text-11 bg-blue-50 text-blue-600 px-2 py-0.5 rounded-full font-medium" title="支持工具调用和图像生成">
                            <Wrench className="w-3 h-3" /> 支持全部工具
                          </span>
                        );
                      }
                      return (
                        <span className="inline-flex items-center gap-1 text-11 bg-blue-50 text-blue-600 px-2 py-0.5 rounded-full font-medium" title="支持工具调用，但不支持图像生成等特殊工具">
                          <Wrench className="w-3 h-3" /> 支持基础工具
                        </span>
                      );
                    })()}
                    <button onClick={() => removeProvider(id)} className="text-secondary hover:text-accent-red transition-colors p-1 radius-sm hover:bg-accent-red-dim">
              ...
              <CardContent className="space-y-3">
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <input type="text" value={cfg.base_url} onChange={(e) => updateProvider(id, 'base_url', e.target.value)}
                ...
                <div>
                  ...
                  <div className="relative">
                    <input type={showKeys[id] ? 'text' : 'password'} value={cfg.api_key}
                      onChange={(e) => updateProvider(id, 'api_key', e.target.value)}
                      placeholder="sk-..."
                      className={`w-full px-3 py-2.5 pr-10 border border-divider radius-md text-13 focus:ring-2 focus:ring-[var(--accent-primary)] focus:border-apple-blue outline-none transition-colors bg-white`} />
                    <button type="button" onClick={() => setShowKeys((p) => ({ ...p, [id]: !p[id] }))}
                ...
                <div>
                  <label className="block text-xs font-medium text-secondary mb-1.5">默认模型</label>
                  <input type="text" value={cfg.model} onChange={(e) => updateProvider(id, 'model', e.target.value)}
        ...
        {unconfiguredPresets.length > 0 && (
          <Card>
            <CardHeader>
              <CardTitle className="text-sm">添加提供商</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex flex-wrap gap-2">
                {unconfiguredPresets.map((p) => (
                  <button key={p.id} onClick={() => addPreset(p)}
                    className="px-4 py-2 text-13 bg-secondary border border-divider rounded-full text-primary hover:border-accent-primary hover:bg-white transition-all font-medium">
                    + {p.name}
                  </button>
                ))}
              </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-sm">添加自定义提供商（OpenAI 兼容）</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="flex gap-2">
              <input type="text" value={customName} onChange={(e) => setCustomName(e.target.value)}
                onKeyDown={(e) => e.key === 'Enter' && addCustom()}
                placeholder="提供商名称（如 siliconflow）" className={inputClass} />
